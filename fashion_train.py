...
from losses import LDAMLoss, FocalLoss, SBLoss, EQL
from opts import parser
from sam import SAM
from scipy.interpolate import interp1d
from fashion_test import Net
best_acc1 = 0
import warnings
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'fashion':
        train_dataset = IMBALANCEFASHION(root='./data', imb_type=args.imb_type, imb_factor=args.imb_factor,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.train_rule == 'None':
            ...
            train_sampler = None
            per_cls_weights = 1.0 / np.array(cls_num_list)  # 不同点
            per_cls_weights = per_cls_weights / np.sum(per_cls_weights) * len(cls_num_list)
            per_cls_weights = torch.FloatTensor(per_cls_weights).cuda(args.gpu)
        else:
...
def validate(val_loader, model, criterion, criterion_ib, epoch, args, log=None, tf_writer=None, flag='val'):
    ...
    with torch.no_grad():
        ...
        # print(f1_micro)
        output = (
            '{flag} Results: Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Loss {loss.avg:.5f} F1_micro {F1_micro:.5f} G-Means {Gmean:.5f}'
